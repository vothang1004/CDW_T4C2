...
public class SaleService {
	...
	public List<Order> getOrdersByCategoryAndMonth(Long categoryId, int year, Month month, String paymentMethod) {
		...
		cq.select(orderRoot).distinct(true).where(cb.equal(categoryJoin.get("id"), categoryId),
				...
				cb.notEqual(orderRoot.get("paymentMethod"), paymentMethod));
...
//	    List<Order> orders = orderRepository.findByCategoryIdAndMonth(categoryId, month);
		List<Order> orders = getOrdersByCategoryAndMonth(categoryId, 2023, month, "not payment yet");
